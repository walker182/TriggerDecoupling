...
Note that this file is not the same as 'clean_tuning.py'. The train.tsv and dev.tsv are transfered by chatGPT, while test.tsv is the original
file, because the test.tsv does not matter, we select the best model only according to the dev.tsv file.
'''

# Defend
from convert_defend import backdoorModel
import json
import argparse
...
def parse_args():
    ...
    parser.add_argument('--config_path', type=str, default='./configs/my_config/clean_train_gpt/sst2_addsent.json')
...
def main(config,file):
    ...
    if backdoor_type=='train':
        state_dict = torch.load(backdoorModel[file], map_location={'cuda:0': 'cuda:0','cuda:1':'cuda:0','cuda:2':'cuda:0','cuda:3':'cuda:0'})
