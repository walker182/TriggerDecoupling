...
class Trainer(object):
    ...
    def register(self, model: Victim, dataloader, metrics, defender=None):
        r"""
        Register model, dataloader and optimizer
        """
        self.optimizer_list = []
        if defender is not None:
            self.converter = defender.converter
            self.converter.train()
            self.converter.zero_grad()
            optimizer_converter = AdamW(self.converter.parameters(), lr=self.converter.lr)
            self.optimizer_list.append(optimizer_converter)
        else:
            self.converter = None
        self.model = model
        ...
        self.optimizer_model = AdamW(optimizer_grouped_parameters, lr=self.lr)
        self.optimizer_list.append(self.optimizer_model)

        train_length = len(dataloader["train"])
        self.scheduler_model = get_linear_schedule_with_warmup(self.optimizer_model,
                                                               num_warmup_steps=self.warm_up_epochs * train_length,

                                                               num_training_steps=self.epochs * train_length)
        self.scheduler_list = []
        self.scheduler_list.append(self.scheduler_model)
        if defender is not None:
            scheduler_converter = get_linear_schedule_with_warmup(optimizer_converter,
                                                                  num_warmup_steps=self.warm_up_epochs * train_length,
                                                                  num_training_steps=self.epochs * train_length)
            self.scheduler_list.append(scheduler_converter)

        self.poison_loss_all = []
        self.normal_loss_all = []
        if self.visualize:
            poison_loss_before_tuning, normal_loss_before_tuning = self.comp_loss(model, dataloader["train"], defender)
    ...
    def train_one_epoch(self, epoch: int, epoch_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if self.converter is not None:
                batch_embeds, vae_loss, new_attention_mask, encoder_logits = \
                    self.converter(batch_inputs['input_ids'],
                                   True, self.each_step,
                                   self.model.raw_embed.weight,
                                   batch_inputs['attention_mask'])
                batch_inputs['inputs_embeds'] = batch_embeds
                batch_inputs['attention_mask'] = new_attention_mask
            ...
            if self.converter is not None:
                encoder_loss = self.loss_function(encoder_logits, batch_labels)
                loss = loss + vae_loss + encoder_loss
                # loss = vae_loss+encoder_loss
                # loss = loss
            #     loss = loss-loss1
            ...
            if (step + 1) % self.gradient_accumulation_steps == 0:
                ...
                for opti in self.optimizer_list:
                    opti.step()
    ...
    def train(self, model: Victim, dataset, metrics: Optional[List[str]] = ["accuracy"], defender: Optional = None):
        """
        Train the model.
        ...
        self.register(model, dataloader, metrics, defender)
        ...
        for epoch in range(self.epochs):
            ...
            dev_results, dev_score = self.evaluate(self.model, eval_dataloader, self.metrics, self.converter,
                                                   self.each_step, self.model.raw_embed.weight)

            if self.visualize:
                hidden_state, labels, poison_labels = self.compute_hidden(model, epoch_iterator, defender)
            ...
            if dev_score > best_dev_score:
                ...
                if self.ckpt == 'best':
                    ...
                    if self.converter is not None:
                        torch.save(self.converter.state_dict(), self.model_checkpoint('converter_best'))
        ...
        if self.ckpt == 'last':
            torch.save(self.model.state_dict(), self.model_checkpoint(self.ckpt))
            if self.converter is not None:
                torch.save(self.converter.state_dict(), self.model_checkpoint('converter_last'))
        ...
        if self.converter is not None:
            state_dict_con = torch.load(self.model_checkpoint('converter_' + self.ckpt))
            self.converter.load_state_dict(state_dict_con)
            return self.model, defender
        # test_score = self.evaluate_all("test")
        return self.model

    def evaluate(self, model, eval_dataloader, metrics, converter=None, step=None, weight=None):
        """
        Evaluate the model.
        ...
        """
        results, dev_score = evaluate_classification(model, eval_dataloader, metrics, converter, step, weight)
        return results, dev_score

    def compute_hidden(self, model: Victim, dataloader: torch.utils.data.DataLoader, defender=None):
        """
        ...
        for batch in tqdm(dataloader):
            ...
            if defender is not None:
                batch_embeds = defender.converter(batch_inputs['inputs_embeds'], self.model.raw_embed)
                batch_inputs['inputs_embeds'] = batch_embeds
    ...
    def comp_loss(self, model: Victim, dataloader: torch.utils.data.DataLoader, defender=None):
        poison_loss_list, normal_loss_list = [], []
        for step, batch in enumerate(dataloader):
            batch_inputs, batch_labels = self.model.process(batch)
            if defender is not None:
                # batch_inputs = defender.converter(batch_inputs)
                batch_embeds = defender.converter(batch_inputs['inputs_embeds'], self.model.raw_embed)
                batch_inputs['inputs_embeds'] = batch_embeds
            output = self.model(batch_inputs)
            logits = output.logits
    ...
    def plot_curve(self, davies_bouldin_scores, normal_loss, poison_loss,
                   fig_basepath: Optional[str] = "./learning_curve", fig_title: Optional[str] = "fig"):
    ...
    def save_vis(self):
        ...
        embedding = self.visualization(self.hidden_states, self.labels, self.poison_labels,
                                       fig_basepath=os.path.join('./visualization', self.poison_setting,
                                                                 self.poison_method, str(self.poison_rate)))
